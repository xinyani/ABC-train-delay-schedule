...
def fitness(late_timetable):
    latetime = 0
    for j in range(late_train,train):
        for i in range(2*late_station-2,2 * station):
...
def limit(late_timetable_all):
    for j in range(len(late_timetable_all)):
        ...
        for m in range(late_train, train):  # 如果发车时间晚点，则提速，运行时间按最小运行时间,如果提前到站，则到站时间按正点运行时间
            ...
            for n in range(2*late_station-1, 2 * station - 1, 2):
        ...
        for mm in range(late_train, train):  # 列车不能早发车
            ...
            for nn in range(2*late_station-1, 2 * station - 1, 2):
        ...
        for h in range(2*late_station-2,2*station):
...
def initialization():
    ...
    for i in range(popsize):  # 随机生成列车调整时间
        ...
        late_timetable[:,0:2*late_station-2]=0
