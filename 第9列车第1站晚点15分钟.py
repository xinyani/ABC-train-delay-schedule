...
def limit(late_timetable_all):
    for j in range(len(late_timetable_all)):
        for m in range(late_train, train):  # 如果发车时间晚点，则提速，运行时间按最小运行时间,此时如果提前到站，则到站时间按正点运行时间
        ...
        for h in range(train):  # 保证每列车各自的运行时间递增
            for l in range(2, 2*station - 1):
...
def employed(adjust_timetable,limit_list):
    ...
    # hanshuzhi=[]
    # for i in range(popsize):
    #     # hanshuzhi.append(abs(fitness(adjust_timetable[i])-fitness(adjust_timetable[best_individue_index])))
    #     hanshuzhi.append(fitness(adjust_timetable[i]))
    # hanshuzhi=np.array(hanshuzhi)
    # sorted_indices = np.argsort(hanshuzhi)
    # hanshuzhi = hanshuzhi[sorted_indices]
    # v=vv[sorted_indices]
    # jingying=v[:int(0.1*popsize)]
    ...
    for i in range(popsize):
        ...
        for k in range(late_train,train):
            ...
            #     v[i,k,j]=v[i,k,j]+int((random.uniform(-1,1))*(v[i,k,j]-v[h,k,j]))
                ...
                # else:
                #     hh = random.choice([d for d in range(int(0.1*popsize)) if d != i])
                #     v[i,k,j]=jingying[hh,k,j]+int((random.uniform(-1,1))*(jingying[hh,k,j]-v[i,k,j]))
...
X = np.arange(1, iteration+1).astype(dtype=np.str)
